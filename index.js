...
const { help } = require('./cika/help')
const { rules } = require('./cika/rules')
const { bahasa } = require('./cika/bahasa')
const { donasi } = require('./FADHIL/donasi')
const { limits } = require('./cika/limit')
const { bokep } = require('./cika/bokep.js')
const { ownermenu } = require('./cika/ownermenu.js')
const { groupmenu } = require('./cika/groupmenu.js')
const { mediamenu } = require('./cika/mediamenu.js')
const { nsfwmenu } = require('./cika/nsfwmenu.js')
const { listmenu } = require('./cika/listmenu.js')
const { funmenu } = require('./cika/funmenu.js')
const { makermenu } = require('./cika/makermenu.js')
//jangan hiraukan ini!!,,,ini uji coba yang gagal:) kontol//
//const { gabutmenu } = require('./cika/gabutmenu')
//const { ownermenu } = require('./cika/ownermenu')
//const { groupmenu } = require('./FADHIL/groupmenu')
//const { vipmenu } = require('./cika/vipmenu')
//const { nsfwmenu } = require('./cika/nsfwmenu')
//const { creatormenu } = require('./cika/creatormenu')
//const { downloadmenu } = require('./cika/downloadmenu')
//const { allmenu } = require('./cika/allmenu')
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const AgusAliansyah = ["62895603272884@s.whatsapp.net"]
			...
			const ownerNumber = [
			"62895603272884@s.whatsapp.net"//GANTI NOMOR LU
			]
			premium = [
			"62895603272884@s.whatsapp.net","6283102650464@s.whatsapp.net"//GANTI NOMOR YAG MAU DI PREM, GABISA NYIMPEN DI DATABASE!!
			]
...
/*********END PISAH MENU***********/
           ...
           if (!isUser) return reply(mess.only.userB)
            ...
            break
				...
				case 'owner':
				case 'creator':
                 client.sendMessage(from, {displayname: "+62 895-6032-72884 vcard: vcard}, MessageType.contact, { quoted: mek})
	   ...
	   case 'coli':
		    ...
		    case 'map':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘CIKABOT〙', members_id, true)
					break
			    case 'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				case 'clearall':
					...
					reply(`\`\`\`Sukses delete all chat Cikabot\`\`\``)
					break
                                case 'bcgc':
				...
				default:
                  ...
                  reply(`Maaf Kak *${pushname2}*, Command *${prefix}${command}* Tidak Terdaftar Di Dalam *${prefix}menu!*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[CIKABOT]','Yellow'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'yellow'))
